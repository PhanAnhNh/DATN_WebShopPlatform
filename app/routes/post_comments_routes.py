...
async def get_comments(post_id: str, db = Depends(get_database)):
    ...
    # THÊM ĐOẠN NÀY ĐỂ ÉP KIỂU OBJECTID SANG STRING
    for comment in comments:
        # Xử lý post_id
        if "post_id" in comment:
            comment["post_id"] = str(comment["post_id"])
            
        # Xử lý luôn _id của comment (phòng trường hợp lỗi tương tự với _id)
        if "_id" in comment:
            comment["_id"] = str(comment["_id"])
            
        # Nếu model PostCommentResponse của bạn yêu cầu trả về key là 'id' thay vì '_id'
